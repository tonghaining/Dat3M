...
public class VisitorOpsBitsTest {
    ...
    public void testOpsBitwiseAndVector() {
        ...
        for (int i = 0; i < 3; i++) {
            IntBinaryExpr binExpr = (IntBinaryExpr) expr.getOperands().get(i);
            assertEquals(builder.getExpression("%value1").getOperands().get(i), binExpr.getLeft());
            assertEquals(builder.getExpression("%value2").getOperands().get(i), binExpr.getRight());
            assertEquals(AND, binExpr.getKind());
        }
    }

    @Test
    public void testOpsBitwiseAndVectorOperandUnMatchSize() {
        // given
        MockProgramBuilderSpv builder = new MockProgramBuilderSpv();
        builder.mockIntType("%int", 64);
        builder.mockVectorType("%vector", "%int", 3);
        builder.mockVectorType("%vector4", "%int", 4);
        builder.mockConstant("%value1", "%vector", List.of(1, 2, 3));
        builder.mockConstant("%value2", "%vector4", List.of(5, 6, 7, 8));
        String input = "%reg = OpBitwiseAnd %vector %value1 %value2";
        ...
        try {
            ...
            assertEquals("Illegal definition for '%reg', " +
                            "operand '{ bv64(5), bv64(6), bv64(7), bv64(8) }' must be of type '[3 x bv64]'",
                    e.getMessage());
        }
    }

    @Test
    public void testOpsBitwiseAndVectorResultUnMatchSize() {
        // given
        MockProgramBuilderSpv builder = new MockProgramBuilderSpv();
        builder.mockIntType("%int", 64);
        builder.mockVectorType("%vector", "%int", 3);
        ...
        builder.mockConstant("%value1", "%vector", List.of(1, 2, 3));
        builder.mockConstant("%value2", "%vector", List.of(5, 6, 7));
        String input = "%reg = OpBitwiseAnd %vector4 %value1 %value2";
        ...
        try {
            ...
            // then
            assertEquals("Illegal definition for '%reg', " +
                            "operand '{ bv64(1), bv64(2), bv64(3) }' must be of type '[4 x bv64]'",
                    e.getMessage());
        }
    }
    ...
    public void testOpsShiftInvalidOperandType() {
        ...
        try {
            ...
            // then
            assertEquals("Illegal definition for '%reg', " +
                            "operand '{ bv64(1), bv64(2), bv64(3) }' must be of type 'bv64'",
                    e.getMessage());
        }
    }
