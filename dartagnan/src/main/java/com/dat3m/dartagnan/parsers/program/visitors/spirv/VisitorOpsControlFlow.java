...
public class VisitorOpsControlFlow extends SpirvBaseVisitor<Event> {
    ...
    public Event visitOpLifetimeStart(SpirvParser.OpLifetimeStartContext ctx) {
        ...
        if (pointerVariable.getAddress() instanceof VirtualMemoryObject virtualMemoryObject) {
            builder.allocateVariable(pointerId, size, virtualMemoryObject);
        } else {
            builder.addExpression(pointerId, pointerVariable);
        }
        return null;
    }
