...
public class VisitorOpsConversion extends SpirvBaseVisitor<Void> {
    ...
    public Void visitOpBitcast(SpirvParser.OpBitcastContext ctx) {
        ...
        if (builder.getType(typeId) instanceof ScopedPointerType pointerType) {
            ...
            int size = types.getMemorySizeInBytes(pointerType);
            if (!(memObj instanceof VirtualMemoryObject vmo)) {
                throw new UnsupportedOperationException("OpBitcast requires a virtual memory object");
            }
            builder.allocateVariable(id, size, vmo);
