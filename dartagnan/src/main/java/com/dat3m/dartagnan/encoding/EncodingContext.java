...
public final class EncodingContext {
    ...
    private BitvectorFormula toBitvector(Formula formula, int length) {
        ...
        if (formula instanceof TupleFormula f) {
            List<Formula> elements = f.getElements();
            BitvectorFormula result = (BitvectorFormula) elements.get(0);
            for (Formula bit : elements.subList(1, elements.size())) {
                result = bvmgr.or(result, (BitvectorFormula) bit);
            }
            return result;
        }
